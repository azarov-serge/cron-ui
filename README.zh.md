...
| `CronExpressionField` | `src/features/cron/components/CronEditor/components/CronFields/` — cron 表达式与复制 |
| `CronDescriptionField` | `src/features/cron/components/CronEditor/components/CronFields/` — 可读描述 |
...
├── features/cron/
│   ├── components/
│   │   ├── CronEditor/       # CronEditor、CronFields/、各区块
│   │   └── CronChecker/
│   ├── hooks/
│   ├── pages/CronPage.tsx
│   └── utils/
...
三个标签页，状态与 URL 同步（`?cron=0+9+*+*+1`、`?tab=checker`）：

| 标签 | URL | 内容 |
| ---- | --- | ---- |
| Cron 构造器 | _(默认)_ | 内联 `CronEditor`（底部为 cron 与描述） |
| 编辑器参数 | `editorParams` | `CronOptions` 面板 |
| Cron 校验 | `checker` | `CronChecker`，**修改 cron** → 将解析结果带到构造器 |

- `CronEditor` 的修改通过 `onChange` 立即写入 URL。
- 校验页的 **修改 cron** 将有效表达式带到构造器。
- **复制** — 移动端带本地化标签的 `TextButton`。
- 语言（`cron-ui-locale`）与主题（`cron-ui-theme`）保存在 `localStorage`；首次访问时尝试使用浏览器语言。

## CronEditor

属性：`cron`、`onChange`、`options`（`CronOptions`）。

每次修改计划都会调用 `onChange`（无需保存按钮）。
