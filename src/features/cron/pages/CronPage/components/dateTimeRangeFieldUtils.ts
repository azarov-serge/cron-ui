...
/** UTC ms → UI строки в Europe/Moscow */
export const utcMillisToMoscowWallClock = (
  utcMillis: number,
): MoscowWallClock => {
  const parts = getZonedDateParts(new Date(utcMillis), MOSCOW_TIME_ZONE);
...
/**
 * Абсолютный max: min(prop maxDate, сейчас по Москве с floor minuteStep).
 */
export const resolveMaxUtcMillis = (
  maxDate: Date | undefined,
  minuteStep: number,
  now: Date = new Date(),
): number => {
  const nowUtc = getMoscowNowUtcMillis(minuteStep, now);

  if (!maxDate) {
    return nowUtc;
  }

  return Math.min(maxDate.getTime(), nowUtc);
};

/**
 * Clamp к resolved max (prop maxDate ∩ сейчас по Москве).
 */
export const clampToResolvedMax = (
  dateValue: string,
  timeValue: string,
  maxDate: Date | undefined,
  minuteStep: number,
  ...
  const iso = dateTimeToMoscowISO(dateValue, timeValue);
  const maxUtc = resolveMaxUtcMillis(maxDate, minuteStep, now);

  if (!iso) {
    return clampToMoscowNow(dateValue, timeValue, minuteStep, now);
  }
  ...
  const wall = utcMillisToMoscowWallClock(maxUtc);

  return { date: wall.date, time: wall.time, changed: true };
};

/** true, если UI date+time (как MSK) позже разрешённого max */
export const isAfterResolvedMax = (
  dateValue: string,
  timeValue: string,
  maxDate: Date | undefined,
  minuteStep: number,
  ...
  return Date.parse(iso) > resolveMaxUtcMillis(maxDate, minuteStep, now);
};
